...
核心优势写 2-3 条，潜在短板写 2-3 条，都使用上述分点格式。
内在张力写一整段话，不拆分条目。
...
包含四个子项，每个子项标题独占一行（格式同上：**标题**）。
子项内容如果只有一个观点，直接写在标题同行（**标题**：内容）。
如果包含多个观点（如多个岗位方向），则在标题下方用分点格式逐条展开。

四个子项：
**适合的公司类型**：文化氛围、公司阶段、团队规模，要具体到可执行（如"B轮到D轮的科技公司"而非"创新型公司"）
**适合的团队角色**：在团队中应扮演什么位置，为什么
**理想搭档画像**：需要什么性格特质的上级和同事来互补，说明原因
**最适合的岗位类型**：结合简历背景给出 2-3 个具体岗位方向，每个方向用一个分点
