...
- 禁止使用列表符号（* - 1. 等），用分点格式代替列表
- 分点格式：每个要点为「**要点标题**：描述内容」，标题和描述在同一段，要点之间用空行分隔
...
核心优势写 2-3 条，潜在短板写 2-3 条，都使用分点格式。
内在张力深入剖析此人性格中最有趣的矛盾组合，这是整份报告最体现洞察力的部分。如果存在多个张力点，也使用分点格式。

不要泛泛说"你很有创造力"，要结合简历经历说明优势如何具体体现。短板要指出真实风险，不要敷衍。

【板块二：最佳战场】

包含四个子项，每个子项标题独占一行（**标题**，不带冒号和内容）。
每个子项下方用分点格式展开具体观点。

四个子项：
**适合的公司类型** — 文化氛围、公司阶段、团队规模，要具体到可执行（如"B轮到D轮的科技公司"而非"创新型公司"），多种类型分别列为独立分点
**适合的团队角色** — 在团队中应扮演什么位置，为什么
**理想搭档画像** — 需要什么性格特质的上级和同事来互补，说明原因
**最适合的岗位类型** — 结合简历背景给出 2-3 个具体岗位方向，每个方向为一个独立分点

【板块三：成长引擎】

严格包含三个子模块，每个用 ### 作标题（三个 # 号 + 空格 + 标题，不要添加额外的 # 号）。禁止空泛鸡汤，不要再添加 ### 或 ## 级别的标题。
每个子模块内容如果包含多个建议，使用分点格式逐条展开。
